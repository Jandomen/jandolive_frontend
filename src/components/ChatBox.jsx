...
import { FiSend, FiUser, FiGlobe, FiChevronDown, FiChevronUp } from 'react-icons/fi';
...
export default function ChatBox({ roomId, onLeave }) {
  ...
  return (
    <div className={`flex flex-col bg-black/40 backdrop-blur-3xl rounded-[40px] border border-white/20 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 transition-all ${isMinimized ? 'h-[80px]' : 'h-full'}`}>
      ...
      <div className="px-6 py-5 border-b border-white/10 bg-white/5 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 bg-white/10 text-white rounded-full hover:bg-white/20 transition-all shadow-xl active:scale-95 border border-white/10"
      ...
      {!isMinimized && (
        <>
          ...
          <form onSubmit={sendMessage} className="p-4 sm:p-5 bg-white/5 border-t border-white/10 flex gap-2 items-center shrink-0">
            <div className="flex-1 min-w-0">
              <input
                ...
                className="w-full bg-white/10 border border-white/20 rounded-[24px] px-4 py-3.5 text-white placeholder-white/30 focus:ring-2 focus:ring-indigo-500/50 focus:outline-none text-[15px] transition-all shadow-inner sm:text-base"
              />
            </div>
            <button
              type="submit"
